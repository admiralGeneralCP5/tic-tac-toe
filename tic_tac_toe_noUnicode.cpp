...
int main() {
    ...
    while (true) {
        ...
        while (true) {
            ...
            // makes sure that moves are no overwritten
            bool moveMade = false;
            string current_move = asciiMove(number, letter); 
            for (auto move : used_moves) {
            ...
            // input is valid
            num_moves++; // if there is no winner, this will be used to stop when the grid is full
        ...
        if (player == 'X') {
            ...
            used_moves.push_back(coordinate);
        } else {
